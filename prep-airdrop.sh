...
finalize_external_keychain() {
  ...
  # Set correct permissions on keychain if it was copied
  if [[ -f "${OUTPUT_PATH}/config/${EXTERNAL_KEYCHAIN}-db" ]]; then
...
copy_with_manifest "${SCRIPT_SOURCE_DIR}/scripts/server/operator-first-login.sh" "scripts/operator-first-login.sh" "OPTIONAL"
if [[ -f "${OUTPUT_PATH}/scripts/operator-first-login.sh" ]]; then
...
if [[ -d "${SCRIPT_SOURCE_DIR}" ]]; then
  ...
  copy_with_manifest "${SCRIPT_SOURCE_DIR}/config/casks.txt" "config/casks.txt" "OPTIONAL"
  copy_with_manifest "${SCRIPT_SOURCE_DIR}/config/logrotate.conf" "config/logrotate.conf" "OPTIONAL"
...
README_SOURCE="${SCRIPT_SOURCE_DIR}/docs/setup/firstboot-README.md"
if [[ -f "${README_SOURCE}" ]]; then
  echo "Processing README file..."
  sed "s/\${SERVER_NAME_LOWER}/${SERVER_NAME_LOWER}/g" \
    "${README_SOURCE}" >"${OUTPUT_PATH}/README.md"
  add_to_manifest "README.md" "OPTIONAL"
  echo "✅ README creation"
else
  collect_warning "firstboot-README.md not found in source directory"
