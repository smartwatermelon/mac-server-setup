...
if [[ -z "${MONITORING_EMAIL:-}" ]] || [[ "${MONITORING_EMAIL}" == "your-email@example.com" ]]; then
  echo "Error: MONITORING_EMAIL is not configured in ${CONFIG_FILE}"
  echo "Please set it to a valid email address before running this script."
  exit 1
fi
...
if [[ "${APP_PASSWORD:-}" == "__EXISTING__" ]]; then
  ...
  # The noreply@hostname value is cosmetic and won't appear in delivered mail.
...
# The noreply@${HOSTNAME_LOWER} value below is cosmetic only.
#
