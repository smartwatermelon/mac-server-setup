...
if command -v msmtp &>/dev/null; then
  log "msmtp already installed: $(msmtp --version | head -1 || true)"
else
  log "Installing msmtp via Homebrew..."
  if brew install msmtp; then
    log "msmtp installed: $(msmtp --version | head -1 || true)"
  else
...
if command -v jq &>/dev/null; then
  log "jq already installed: $(jq --version || true)"
else
  log "Installing jq via Homebrew..."
  if brew install jq; then
    log "jq installed: $(jq --version || true)"
  else
...
# Generated by msmtp-setup.sh on $(date '+%Y-%m-%d' || true)
#
