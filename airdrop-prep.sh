...
if [[ -n "$GITHUB_REPO" && "$GITHUB_REPO" != "https://github.com/yourusername/tilsit-setup.git" ]]; then
  ...
  cat > "$OUTPUT_PATH/scripts/first-boot.sh" << 'EOF'
#!/bin/bash
#
...
if [ -d "$SETUP_DIR/ssh_keys" ]; then
  log "Found SSH keys in setup directory"
  ...
  if [ -f "$SETUP_DIR/ssh_keys/authorized_keys" ]; then
    log "Copying authorized_keys for admin user"
    cp "$SETUP_DIR/ssh_keys/authorized_keys" "$ADMIN_SSH_DIR/"
    chmod 600 "$ADMIN_SSH_DIR/authorized_keys"
    check_success "Admin SSH key setup"
  fi
else
  log "No SSH keys found in setup directory - manual key setup will be required"
fi
...
if dscl . -list /Users | grep -q "^$OPERATOR_USERNAME$"; then
  ...
  if [ -d "$SETUP_DIR/ssh_keys" ] && [ -f "$SETUP_DIR/ssh_keys/operator_authorized_keys" ]; then
    OPERATOR_SSH_DIR="/Users/$OPERATOR_USERNAME/.ssh"
    log "Setting up SSH keys for operator account"
    
    sudo mkdir -p "$OPERATOR_SSH_DIR"
    sudo cp "$SETUP_DIR/ssh_keys/operator_authorized_keys" "$OPERATOR_SSH_DIR/authorized_keys"
...
exit 0
EOF
  chmod +x "$OUTPUT_PATH/scripts/first-boot.sh"
  
  # Create second-boot.sh (abbreviated version, just a reference)
  cat > "$OUTPUT_PATH/scripts/second-boot.sh" << 'EOF'
#!/bin/bash
#
...
exit 0
EOF
  chmod +x "$OUTPUT_PATH/scripts/second-boot.sh"
  
  # Create formulae.txt
  cat > "$OUTPUT_PATH/lists/formulae.txt" << 'EOF'
bash
bash-completion@2
bat
...
EOF
  ...
  # Create casks.txt
  cat > "$OUTPUT_PATH/lists/casks.txt" << 'EOF'
1password
1password-cli
appcleaner
backblaze
...
EOF
  ...
  echo "Scripts and lists created directly in output directory"
fi
