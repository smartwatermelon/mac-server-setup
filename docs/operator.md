...
After the first reboot, the Mac Mini automatically logs in as the **operator** user. This account is designed for day-to-day server operations and application management.
...
If automatic login didn't work:
1. **Select "operator" from user list** on login screen
2. **Password is stored in 1Password**: `op://personal/TILSIT operator/password`
3. **Retrieve password** from 1Password on your development Mac if needed
...
**TouchID Sudo Access**: Use TouchID for administrative tasks
...
**SSH Method**: `ssh admin@tilsit.local` from your development Mac
...
# Add to ~/.bash_profile
alias ll='ls -la'
alias logs='tail -f ~/.local/state/tilsit-setup.log'
...
```bash
ping google.com
ssh admin@tilsit.local 'echo SSH working'
```

### Log Files

**Setup logs**: `~/.local/state/tilsit-setup.log`
...
ssh-keygen -t ed25519 -f ~/.ssh/id_ed25519_operator -C "operator@tilsit"
...
**TouchID is enabled** for sudo commands. For remote SSH sessions, you'll need to enter the operator password.

**Password location**: `op://personal/TILSIT operator/password` in 1Password
...
**Logs**: Most issues are logged in `~/.local/state/tilsit-setup.log`
